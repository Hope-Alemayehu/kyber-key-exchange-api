...
if not OQS_AVAILABLE:
    logger.warning("OQS not available. Using simulated cryptography (NOT SECURE FOR PRODUCTION)")
    logger.warning("To enable real post-quantum cryptography, please install the required dependencies.")
    ...
    try:
        ...
        if not OQS_AVAILABLE:
            ...
            with oqs.KeyEncapsulation(KEM_ALG, private_key) as kem:
                ...
                logger.debug(f"Decapsulated shared secret length: {len(shared_secret)} bytes")
