...
app = FastAPI(title="Kyber Key Exchange API",
...
@app.get("/")
async def root():
    return {"message": "Kyber Key Exchange API - Use /docs for interactive documentation"}
...
async def decrypt_shared_secret(request: DecapsulateRequest):
    ...
    try:
        ...
        if "private_key" not in session:
            raise HTTPException(status_code=400, detail="No private key found in session")
